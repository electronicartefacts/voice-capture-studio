...
export async function sha256Blob(blob: Blob): Promise<string> {
  ...
  const webCryptoDigest = await digestWithWebCrypto(buffer);

  return webCryptoDigest ?? sha256Bytes(new Uint8Array(buffer));
}

export function sha256Bytes(bytes: Uint8Array): string {
  const paddedBytes = createPaddedMessage(bytes);
  ...
  for (let offset = 0; offset < paddedBytes.length; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      ...
      words[index] =
        (paddedBytes[wordOffset] << 24) |
        (paddedBytes[wordOffset + 1] << 16) |
        (paddedBytes[wordOffset + 2] << 8) |
        paddedBytes[wordOffset + 3];
    }
...
async function digestWithWebCrypto(
  buffer: ArrayBuffer,
): Promise<string | null> {
...
function createPaddedMessage(bytes: Uint8Array): Uint8Array {
  ...
  writeUint32BigEndian(paddedBytes, paddedLength - 4, bitLength >>> 0);
